...
<template>
    <fieldset class="bg-light-400 flex w-full justify-around">
        <legend class="bg-gray-900/80 px-2 ml-2 rounded">Choose day mode</legend>
        <input id="single-mode" v-model="currentMode" class="hidden" value="single" name="mode" type="radio" />
        <label
            :class="{
                'mode--active border-r-4': currentMode === 'single',
                'mode--passive': currentMode !== 'single',
            }"
            class="toggle-label ml-2 rounded-l-2xl border-3 border-r-0"
            for="single-mode"
            >Single day</label
        >
        ...
        <label
            :class="{
                'mode--active border-l-4': currentMode === 'multi',
                'mode--passive': currentMode !== 'multi',
            }"
            class="toggle-label mr-2 rounded-r-2xl border-3 border-l-0"
            for="multi-mode"
            >Multi day</label
        >
    </fieldset>
