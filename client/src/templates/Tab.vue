...
const props = defineProps({
  tabData: { type: Array<{ tabName: string; componentName: string }>, required: true },
...
<template>
  <div :ref="getRootRef" class="tab">
    <div class="flex h-a">
      <div
        v-for="(tabName, idx) in getTabNames" :key="tabName" :ref="getToggleElement(idx)"
        :class="getCurrentClasses(idx)" class="tab__toggle"
      >
        {{ tabName }}
      </div>
    </div>
