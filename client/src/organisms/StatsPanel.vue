...
<template>
    <div class="stats">
        <div class="clear-btn button-cyberpunk" @click="StoreUser.clearLocalStorage">Clear</div>

        <div class="task-wrapper neon-shadow">
            <p class="task-wrapper__info">{{ getCurrentTaskAmount }}/ {{ tasks.length }}</p>
        </div>

        <div class="calc-btn button-cyberpunk" @click="$router.push('/panel')">Panel</div>
        <div class="calc-btn button-cyberpunk" @click="$router.push('/auth')">Auth</div>
    </div>
</template>
...
<style lang="scss" scoped>
    ...
    .task-wrapper {
        ...
        box-shadow: rgba(0, 0, 0, 0.6) 0px 3px 6px, rgba(0, 0, 0, 0.63) 0px 3px 6px;
    }
    .task-wrapper__info {
