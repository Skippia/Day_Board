class TooltipConstructor {
    run({ error, messageSuccess, messageFail }: { error: boolean; messageSuccess: string; messageFail: string }): void {
        if (!error) return this.draw({ status: 'success', message: messageSuccess })
