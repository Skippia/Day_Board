...
const useTableClickAction = (
  { isClickElement, generateColor, updateCurrentDay, currentList }:
  { isClickElement: (target: HTMLElement) => boolean; generateColor: () => string; updateCurrentDay: Function; currentList: TTask[] }) => {
  ...
  const actionOnClickTableCell = createActionOnClickTableCell(changeCurrentTaskAmount(generateColor, updateCurrentDay))
