...
export function useDOMDatePicker(storeDatePicker: ReturnType<typeof useStoreDatePicker>) {
    ...
    function switchPreviousMonth() {
        ...
        if (getCurrentDate.value.month < 0)
            storeDatePicker.updateCurrentDate({
                month: 11,
                year: getCurrentDate.value.year - 1,
            })
    }
    ...
    const getAmountDaysInCurrentMonth = computed(() =>
        new Date(getCurrentDate.value.year, getCurrentDate.value.month + 1, 0).getDate()
    )
